...
export class OrdenesVentaService {
  ...
  async findCatalogos(args?: FindOrdenesArgs) {
    ...
    const productosConPrecioMinimo = await Promise.all(
    ...
    return {
      clientes,
      productos: productosConPrecioMinimo,
      terminos_pago: terminosPago,
      estados,
      cotizaciones,
    };
  ...
  async create(dto: CreateOrdenVentaDto) {
    return this.prisma.$transaction(async (tx) => {
      ...
      // await this.assertPreciosVentaDetalleValidos(
      //   tx,
      //   detalleFinal,
      //   Number(dto.id_bodega),
      // );
